...
echo "== Binary checks =="
"${ENGINE_CMD}" exec "${IMAGE}" bash -lc 'command -v dropbear && command -v sshd && command -v jupyter && command -v python3 && command -v code-server && test -x /usr/lib/rstudio-server/bin/rserver && printf "%s\n" /usr/lib/rstudio-server/bin/rserver'
